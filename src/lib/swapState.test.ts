...
describe("swap state helpers", () => {
  ...
  it("replaces only the saved swap for the same barcode", () => {
    const existing = {
      "00000001": "sparkling-water",
      "12345678": alternatives[0].id,
    };

    expect(acceptSwap(existing, "12345678", alternatives[1])).toEqual({
      "00000001": "sparkling-water",
      "12345678": "late-july-sea-salt",
    });
  });
