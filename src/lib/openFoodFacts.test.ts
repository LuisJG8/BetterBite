...
describe("Open Food Facts normalization", () => {
  ...
  it("truncates remote text fields without splitting surrogate pairs", () => {
    const product = normalizeOpenFoodFactsResponse(
      {
        code: "12345678",
        status: 1,
        product: {
          product_name: `${"A".repeat(159)}🍎`,
          nutriments: {
            emoji_text: `${"B".repeat(199)}🍎`,
          },
        },
      },
      "12345678",
    ...
    expect(product.name).toBe(`${"A".repeat(159)}🍎`);
    expect(product.nutriments.emoji_text).toBe(`${"B".repeat(199)}🍎`);
  });
