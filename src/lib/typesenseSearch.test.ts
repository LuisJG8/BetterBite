...
describe("typesense food search helpers", () => {
  ...
  it("allows generated local food image assets", () => {
    expect(
      mapTypesenseFoodDocument({
        id: "food-003",
  ...
  it("moves through empty, loading, success, no result, and error states", () => {
    ...
    expect(success.selectedResult).toBeNull();

    const selected = reduceFoodSearchState(success, { type: "resultSelected", result });
    expect(selected.selectedResult).toEqual(result);

    const detailClosed = reduceFoodSearchState(selected, { type: "detailClosed" });
    expect(detailClosed.selectedResult).toBeNull();
