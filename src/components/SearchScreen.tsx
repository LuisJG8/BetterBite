...
export function SearchScreen() {
  ...
  if (state.selectedResult) {
    return <FoodDetailScreen result={state.selectedResult} onBack={() => dispatch({ type: "detailClosed" })} />;
  }
      ...
      <div className="sticky top-0 z-10 border-b border-[#BAC9C9]/40 bg-[#F8FAFB]/95 px-5 py-4 backdrop-blur">
        <form className="flex items-center gap-2" onSubmit={handleSubmit}>
          ...
          <button
            ...
            className="flex h-[47px] w-[52px] shrink-0 items-center justify-center rounded-[16px] bg-[#00696B] text-white shadow-[0_10px_22px_rgba(0,105,107,0.22)] transition hover:bg-[#005B5D] active:scale-95 disabled:cursor-not-allowed disabled:bg-[#7A8587] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00C5C8]/45"
...
function FoodDetailScreen({ result, onBack }: { result: FoodSearchResult; onBack: () => void }) {
  return (
    <div className="-mx-5 min-h-full bg-white px-5 pb-10 pt-6">
      <button
        type="button"
        className="mb-5 inline-flex h-11 items-center gap-2 rounded-full bg-[#EEF7F8] px-3 pr-4 text-sm font-black text-[#3B4949] transition hover:bg-[#DDE8E9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00C5C8]/35"
        onClick={onBack}
      ...
      <div className="mb-5">
        <div className="min-w-0">
          ...
          <h2 className="mt-2 text-[32px] font-black leading-9 text-[#191C1D]">{result.name}</h2>
          <p className="mt-2 text-[17px] font-bold leading-6 text-[#566164]">
            {result.brand} · {result.category}
          </p>
        </div>
      </div>

      <FoodImage result={result} size="detail" />
...
function FoodImage({ result, size }: { result: FoodSearchResult; size: "list" | "detail" }) {
  const isDetail = size === "detail";
  const fallbackInitial = result.name.charAt(0).toUpperCase();

  return (
    <div
      className={`${
        isDetail ? "mb-5 aspect-[4/3] w-full rounded-[22px]" : "h-24 w-24 rounded-[20px]"
      } flex shrink-0 items-center justify-center overflow-hidden border border-[#DDE8E9] bg-[#DDF7EF] text-[22px] font-black text-[#00696B]`}
    >
      {result.imageUrl ? (
        <img
          className="h-full w-full object-cover"
          src={result.imageUrl}
          alt={isDetail ? result.name : ""}
